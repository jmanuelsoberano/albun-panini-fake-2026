...
test('tournament routes expose groups, scoreboards, scorers, and bracket rounds', async ({
  ...
  await expect(page.locator('body')).toContainText('México');
  ...
  await expect(page.getByRole('link', { name: 'México', exact: true })).toBeVisible();
  ...
  await expect(page.locator('body')).toContainText('México');
  await expect(page.getByRole('link', { name: 'Ver cromos en álbum' })).toBeVisible();

  await page.goto(`${origin}/coleccion?q=M%C3%A9xico&qa=tournament`, {
    waitUntil: 'networkidle',
  });
  await expect(page.locator('input[type="search"]')).toHaveValue('México');
